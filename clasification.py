...
data_file = '../data_set_IVa_aa_cnt.txt'
marker_file = '../data_set_IVa_aa_mrk.txt'
...
for train_index,test_index in kf.split(dictionary_wavelet):
	X_train, X_test = dictionary_wavelet[train_index],dictionary_wavelet[test_index]
	y_train, y_test = ywe[train_index],ywe[test_index]
	neigh.fit(X_train,y_train)
	print(accuracy_score(y_test,neigh.predict(X_test)))
clf = KNeighborsClassifier()
model = BaggingClassifier(base_estimator=clf,random_state=7)
results = model_selection.cross_val_score(model, dictionary_wavelet, ywe, cv=kf)
print(results.mean())
clf = KNeighborsClassifier()
...
#dictionary_bandpower

print('********** bandpower ***********')
print('----------Kmeans---------------')
X_sparse = coo_matrix(dictionary_bandpower)
dictionary_bandpower, X_sparse, ywe = resample(dictionary_bandpower, X_sparse, targets, random_state=0)
neigh = KNeighborsClassifier(n_neighbors=3)
kf = KFold(n_splits=10,random_state = 30, shuffle = True)
kf.get_n_splits(dictionary_bandpower)
for train_index,test_index in kf.split(dictionary_bandpower):
	X_train, X_test = dictionary_bandpower[train_index],dictionary_bandpower[test_index]
	y_train, y_test = ywe[train_index],ywe[test_index]
	neigh.fit(X_train,y_train)
	print(accuracy_score(y_test,neigh.predict(X_test)))
clf = svm.SVC()
